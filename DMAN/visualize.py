...
parser.add_argument('dataset', type=str, choices=['ucf101', 'something-v1', 'diving48'])
...
parser.add_argument('--resi', dest='resi', action='store_true',help='open residual connection or not',default=False)
...
if args.modality=='RGB':
    if 'gst' in args.arch:
        model = TemporalModel(num_class, args.num_segments, model = 'GST', backbone=args.arch, alpha = args.alpha, beta = args.beta, 
            dropout = args.dropout, target_transforms = target_transforms,resi=args.resi)
    ...
    def forward(self, x):
        ...
        for i, (name, param) in enumerate(self.submodule.named_parameters()):
            if '.conv3d' in name and 'weight' in name :
                ...
                k_c,k_w, k_h = param.size()[2],param.size()[4], param.size()[3] 
...
x=Image.open(path+args.rgb_prefix.format(1)).convert('RGB').resize((256,256)).crop((16,16,240,240))
x=torch.from_numpy(np.asarray(x)).permute(2, 0, 1).contiguous()
x=torch.unsqueeze(x.float().div(255),1)
for i in range(5,33,4):
    img=Image.open(path+args.rgb_prefix.format(i)).convert('RGB').resize((256,256)).crop((16,16,240,240))
...
x1=x
print((x1.size()))
x1 = x1[0][0].reshape(-1,1,x1.size()[3],x1.size()[4])  # C，B, H, W  ---> B，C, H, W
    ...
    if '0' == name1:        #use for resnet
        for name2, module2 in module1.named_children():
            if 'conv1' in name2:
                ...
                x1 = x1[0][0].reshape(-1,1,x1.size()[3],x1.size()[4])  # C，B, H, W  ---> B，C, H, W
                img_grid = vutils.make_grid(x1, normalize=True, scale_each=True, nrow=4)  # normalize进行归一化处理
                writer.add_image(f'layer1_{name1}{name2}_feature_maps', img_grid, global_step=0)
                print('name2: %s size: %s'%(['layer1',name1,name2],x1.size()))
    ...
    if '0' == name1:        #use for resnet
        for name2, module2 in module1.named_children():
            if 'conv1' in name2:
                ...
                x1 = x1[0][0].reshape(-1,1,x1.size()[3],x1.size()[4])  # C，B, H, W  ---> B，C, H, W
                img_grid = vutils.make_grid(x1, normalize=True, scale_each=True, nrow=4)  # normalize进行归一化处理
                writer.add_image(f'layer2_{name1}{name2}_feature_maps', img_grid, global_step=0)
                print('name2: %s size: %s'%(['layer2',name1,name2],x1.size()))


x = model.base_model.layer2(x)
if args.resi:
    x = model.base_model.lateral_connection(x,model.base_model.lateral_layer3,model.base_model.backward_layer3)
...
for name1, module1 in model.base_model.layer3.named_children():
    if '0' == name1:        #use for resnet
        for name2, module2 in module1.named_children():
            if 'conv1' in name2:
                x1=module2(x)
                x1 = x1[0][:512:16].reshape(-1,1,x1.size()[3],x1.size()[4])  # C，B, H, W  ---> B，C, H, W
                img_grid = vutils.make_grid(x1, normalize=True, scale_each=True, nrow=4)  # normalize进行归一化处理
                writer.add_image(f'layer3_{name1}{name2}_feature_maps', img_grid, global_step=0)
                print('name2: %s size: %s'%(['layer3',name1,name2],x1.size()))

x = model.base_model.layer3(x)
if args.resi:
    x = model.base_model.lateral_connection(x,model.base_model.lateral_layer4,model.base_model.backward_layer4)
...
for name1, module1 in model.base_model.layer4.named_children():
    if '0' == name1:        #use for resnet
        for name2, module2 in module1.named_children():
            if 'conv1' in name2:
                x1=module2(x)
                x1 = x1[0][:1024:32].reshape(-1,1,x1.size()[3],x1.size()[4])  # C，B, H, W  ---> B，C, H, W
                img_grid = vutils.make_grid(x1, normalize=True, scale_each=True, nrow=4)  # normalize进行归一化处理
                writer.add_image(f'layer4_{name1}{name2}_feature_maps', img_grid, global_step=0)
                print('name2: %s size: %s'%(['layer4',name1,name2],x1.size()))
