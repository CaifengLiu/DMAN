...
class STM_Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride = 1, downsample = None):
        ...
        self.cstm_conv1 = nn.Conv3d(planes, planes, kernel_size=(3,1,1), groups=planes, padding=(1,0,0), bias=False)
        self.cstm_bn1 = nn.BatchNorm3d(planes)
        self.cstm_conv2 = nn.Conv3d(planes, planes, kernel_size=(1,3,3), padding=(0,1,1),stride=(1,stride,stride), bias=False)
        ...
        self.cmm_conv= nn.Conv3d(channels, channels, kernel_size=(1,3,3), padding=(0,1,1), groups=channels, bias=False)
        self.cmm_conv2= nn.Conv3d(channels, planes, kernel_size=1, stride=(1,stride,stride), bias=False)
    ...
    def forward(self, x):
        ...
        cmm_y=torch.cat((cmm_z,torch.zeros(cmm_x.size(0),cmm_x.size(1),1,cmm_x.size(3),cmm_x.size(4)).cuda()),2)
    ...
    def __init__(self, block, layers, num_classes=1000, resi=False):
        ...
        for name, m in self.named_modules():
            if 'cstm_conv1' in name:
                ch=m.weight.size()[0]
                part1=torch.cat((torch.ones_like(m.weight[:ch//4,:,0:1]),torch.zeros_like(m.weight[:ch//4,:,1:])),2)
                part2=torch.cat((torch.zeros_like(m.weight[ch//4:ch//4*3,:,0:1]),torch.ones_like(m.weight[ch//4:ch//4*3,:,1:2]),torch.zeros_like(m.weight[ch//4:ch//4*3,:,2:])),2)
                part3=torch.cat((torch.zeros_like(m.weight[ch//4*3:,:,:2]),torch.zeros_like(m.weight[ch//4*3:,:,2:])),2)
                m.weight=torch.nn.Parameter(torch.cat((part1,part2,part3),0))
