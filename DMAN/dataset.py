...
class VideoDataset(data.Dataset):
    def __init__(self, root_path, list_file,
                 ...
                 force_grayscale=False, random_shift=True, test_mode=False, num_clips=1,dataset='diving48'):
    ...
    def _sample_indices(self, record):
        ...
        if self.dataset=='diving48': 
            return offsets + self.new_length//2
        return offsets + 1 + self.new_length//2
    ...
    def _get_test_indices(self, record):
        ...
        if self.num_clips == 1:
            offsets = np.array([int(tick / 2.0 + tick * x) for x in range(self.num_segments)]) + 1 + self.new_length//2
    ...
    def get(self, record, indices):
        ...
        if self.num_clips > 1:
            ...
            for seg_ind in indices:
                p = int(seg_ind)-self.new_length//2
                for i in range(self.new_length):
                    seg_imgs = self._load_image(path, p)
                    images.extend(seg_imgs)
                    p+=1
